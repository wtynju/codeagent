...
export class GuardrailEngine {
  ...
  constructor(customRules?: GuardrailRule[]) {
    this.rules = [
      ...
      { pattern: /curl.*-d\s+/, description: 'curl 发送数据', level: 'NEED_APPROVAL' },
      { pattern: /wget.*--post-data/, description: 'wget 发送数据', level: 'NEED_APPROVAL' },
      { pattern: /\bcd\s+(?!\s*$)/, description: '切换目录', level: 'NEED_APPROVAL' },
