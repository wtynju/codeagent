...
export async function searchCode(params: Record<string, unknown>, workDir?: string): Promise<ToolResult> {
  ...
  try {
    const resolvedPath = path.resolve(workDir || process.cwd(), searchPath);
